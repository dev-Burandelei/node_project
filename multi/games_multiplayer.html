...
function updateInput(dt){
    ...
    if(playerIndex === 0){
        if(keys[87]) sendMove(-1) // W
        if(keys[83]) sendMove(1)  // S
    }

    if(playerIndex === 1){
        if(keys[38]) sendMove(-1) // ↑
        if(keys[40]) sendMove(1)  // ↓
    }
}
...
///////////////////// Funções de comunicação entre o servidor///////////////////////////////////////////
socket.on("connect", () => {
    console.log("Conectado ao servidor Socket.IO");
});

// servidor manda quem é player 0 ou 1
socket.on("playerIndex", (index) => {
    playerIndex = index;
    console.log("Você é o jogador:", index);
});
...
function renderGame(state){
    ...
    // paddles
    p1_y = state.paddles[0]
    p2_y = state.paddles[1]
}
